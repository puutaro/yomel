...
func Test_GenArgTableDto(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: []string{
				...
				"-cmd", "aws",
				"--optVertualOpt", "a",
				"--valVertualValue", "--s", "aaa",
				"--opt", "b",
				"--val", "--n", "bbb",
				"--lopVertualLoption", "c",
				"--argVertualArg", "--s", "awsawsaws1",
				"--arg", "--n", "awsawsaws2",
				"--single",
				"--no-quote",
				"-svc", "a3api",
			...
			want: []argtabledtos.ArgTableDto{
				...
				{StageNo: 1, QuoteTypeSignal: argtabledtos.NoQuote},
		...
		},
		{
			...
			want: []argtabledtos.ArgTableDto{
				...
				{StageNo: 0, QuoteTypeSignal: argtabledtos.NoQuote},
				{StageNo: 0, QuoteTypeSignal: argtabledtos.SingleQuote},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := argtabledtos.GenArgTableDto(inputArgs)
			for i := range tt.want {
				tt.want[i].No = i + 1
			}
