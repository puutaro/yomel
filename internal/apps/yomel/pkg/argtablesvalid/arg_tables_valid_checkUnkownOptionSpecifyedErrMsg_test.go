...
func Test_checkUnknownOptionSpecifyedErrMsg(t *testing.T) {
	tests := []struct {
		name      string
		...
		},
		{
			...
			input: []argtables.ArgTable{
				{StageNo: 0, UnknownOption: "--unknown-ctrl"},
			},
		...
		},
		{
			...
			input: []argtables.ArgTable{
				{StageNo: 1, IsStage: true},
				{StageNo: 1, UnknownOption: "--unknown-stage"},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := checkUnknownOptionSpecifyedErrMsg(tt.input)
			if tt.wantError == "" {
